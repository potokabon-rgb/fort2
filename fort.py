...
def init_db():
  ...
  try:
    ...
    cursor.execute(
        "INSERT OR IGNORE INTO settings (key, value) VALUES ('rate_tier_1',"
        " '80.0')"
    )
    cursor.execute(
        "INSERT OR IGNORE INTO settings (key, value) VALUES ('rate_tier_2',"
        " '90.0')"
    )
    cursor.execute(
        "INSERT OR IGNORE INTO settings (key, value) VALUES ('rate_tier_3',"
        " '120.0')"
    )
    cursor.execute(
        "INSERT OR IGNORE INTO settings (key, value) VALUES ('tier_limit_1',"
        " '6.0')"
    )
    cursor.execute(
        "INSERT OR IGNORE INTO settings (key, value) VALUES ('tier_limit_2',"
        " '20.0')"
    )
    cursor.execute(
    ...
    for p_key in photo_keys:
      cursor.execute(
          "INSERT OR IGNORE INTO settings (key, value) VALUES (?, '')", (p_key,)
      )
...
async def send_log(user_id: int, username: str, action: str):
  ...
  try:
    cursor.execute(
        "INSERT INTO system_logs (user_id, username, action) VALUES (?, ?, ?)",
...
async def cmd_start(event: Message | CallbackQuery, state: FSMContext):
  ...
  try:
    cursor.execute(
        "SELECT user_id, referrer_id FROM users WHERE user_id = ?", (user_id,)
    )
...
async def reviews_handler(callback: CallbackQuery, state: FSMContext):
  ...
  photo = get_setting("reviews_photo")
  await edit_or_reply(
      callback,
      text,
...
async def user_profile(callback: CallbackQuery, state: FSMContext):
  ...
  try:
    cursor.execute(
        "SELECT balance, total_deals, completed_deals, joined_date FROM users"
        " WHERE user_id = ?",
        (user_id,),
    )
    res = cursor.fetchone()
  ...
  photo = get_setting("profile_photo")
  await edit_or_reply(
      callback,
      profile_text,
...
async def referral_menu_handler(callback: CallbackQuery, state: FSMContext):
  ...
  photo = get_setting("referral_photo")
  await edit_or_reply(
      callback,
      text,
      reply_markup=menu_button_kb(),
...
async def calculator_start(callback: CallbackQuery, state: FSMContext):
  ...
  photo = get_setting("calculator_photo")
  await edit_or_reply(
      callback,
      text,
      reply_markup=menu_button_kb(),
...
async def process_balance_choice(callback: CallbackQuery, state: FSMContext):
  ...
  if callback.data == "use_balance_yes" and user_balance >= amount_usdt:
    ...
    try:
      ...
      cursor.execute(
          "UPDATE users SET total_deals = total_deals + 1 WHERE user_id = ?",
          (user_id,),
      )
      conn.commit()
    finally:
...
async def take_order_handler(callback: CallbackQuery):
  ...
  try:
    ...
    cursor.execute(
        "UPDATE orders SET status = 'В работе', admin_id = ? WHERE id = ?",
...
async def process_admin_payout_amount(message: Message, state: FSMContext):
  ...
  try:
    cursor.execute(
        "SELECT user_id, amount_rub, amount_usdt FROM orders WHERE id = ?",
        (order_id,),
    )
    res = cursor.fetchone()
    ...
    if sent_rub < expected_rub:
      ...
      cursor.execute(
          "UPDATE orders SET sent_rub = ?, remainder_usdt = ?, status = 'Ожидает"
          " решения по остатку' WHERE id = ?",
      ...
      cursor.execute(
          "UPDATE orders SET sent_rub = ?, status = 'Ожидает подтверждения'"
          " WHERE id = ?",
          (sent_rub, order_id),
      )
      conn.commit()
...
async def process_user_remainder_choice(
  ...
  try:
    cursor.execute(
        "SELECT remainder_usdt, sent_rub FROM orders WHERE id = ? AND user_id ="
        " ?",
        (order_id, user_id),
    )
    res = cursor.fetchone()
    ...
    if action == "balance":
      ...
      cursor.execute(
          "UPDATE settings SET value = CAST(value AS REAL) + ? WHERE key ="
          " 'total_tips_usdt'",
          (remainder_usdt,),
      )
    ...
    cursor.execute(
        "UPDATE orders SET status = 'Ожидает подтверждения' WHERE id = ?",
        (order_id,),
    )
    conn.commit()
  finally:
...
async def confirm_yes_handler(callback: CallbackQuery, state: FSMContext):
  ...
  try:
    cursor.execute(
        "SELECT status, amount_usdt FROM orders WHERE id = ? AND user_id = ?",
        (order_id, user_id),
    )
    res = cursor.fetchone()
    ...
    cursor.execute(
        "UPDATE orders SET status = 'Завершено' WHERE id = ?", (order_id,)
    )
    cursor.execute(
        "UPDATE users SET completed_deals = completed_deals + 1 WHERE user_id ="
        " ?",
        (user_id,),
    )
...
async def exchange_rate_handler(callback: CallbackQuery, state: FSMContext):
  ...
  photo = get_setting("exchange_rate_photo")
  await edit_or_reply(
      callback,
      text,
      reply_markup=menu_button_kb(),
...
async def history_handler(callback: CallbackQuery, state: FSMContext):
  ...
  try:
    cursor.execute(
        "SELECT COUNT(*), SUM(CASE WHEN status = 'Завершено' THEN 1 ELSE 0 END)"
    ...
    cursor.execute(
        "SELECT id, amount_usdt, amount_rub, status FROM orders WHERE user_id ="
  ...
  photo = get_setting("history_photo")
  await edit_or_reply(
      callback,
      text,
      reply_markup=builder.as_markup(),
...
async def view_order_details(callback: CallbackQuery, state: FSMContext):
  ...
  try:
    cursor.execute(
        "SELECT amount_usdt, amount_rub, status, check_link, phone, bank, fio,"
        " created_at FROM orders WHERE id = ? AND user_id = ?",
...
async def show_admin_menu(event: Message | CallbackQuery, state: FSMContext):
  ...
  try:
    ...
    cursor.execute(
        "SELECT COUNT(*) FROM orders WHERE status IN ('Ожидает администратора',"
        " 'Ожидает реквизиты')"
    )
    pending_count = cursor.fetchone()[0]

    cursor.execute(
        "SELECT COUNT(*) FROM orders WHERE status IN ('В работе', 'Ожидает"
        " решения по остатку', 'Ожидает подтверждения')"
    )
  ...
  builder.button(
      text="🖼 Управление фото меню",
      callback_data="adm_photos_menu",
      icon_custom_emoji_id="5992430854909989581",
  )
  builder.button(
      text="Рассылка",
...
async def adm_photos_menu(callback: CallbackQuery, state: FSMContext):
  ...
  builder = InlineKeyboardBuilder()
  builder.button(
      text="🖼 Главное меню", callback_data="adm_set_photo_main_menu_photo"
  )
  builder.button(
      text="👤 Личный кабинет", callback_data="adm_set_photo_profile_photo"
  )
  builder.button(text="🧮 Калькулятор", callback_data="adm_set_photo_calculator_photo")
  builder.button(
      text="📊 Актуальные курсы",
      callback_data="adm_set_photo_exchange_rate_photo",
  )
  builder.button(text="📁 Мои заявки", callback_data="adm_set_photo_history_photo")
  builder.button(text="🎁 Рефералы", callback_data="adm_set_photo_referral_photo")
  builder.button(text="⭐ Отзывы", callback_data="adm_set_photo_reviews_photo")
  builder.button(
  ...
  await edit_or_reply(
      callback,
      "Выберите категорию, для которой хотите изменить или удалить фото:",
      reply_markup=builder.as_markup(),
      state=state,
  )
...
async def adm_set_category_photo_start(callback: CallbackQuery, state: FSMContext):
  ...
  builder.button(
      ...
      callback_data=f"adm_clear_photo_{photo_key}",
      icon_custom_emoji_id="5778527486270770928",
  )
  builder.button(
      text="⬅️ Отмена",
  ...
  category_names = {
      "main_menu_photo": "Главного меню",
      "profile_photo": "Личного кабинета",
      "calculator_photo": "Калькулятора",
      "exchange_rate_photo": "Актуальных курсов",
      "history_photo": "Моих заявок",
      "referral_photo": "Рефералов",
      "reviews_photo": "Отзывов",
  }
  cat_name = category_names.get(photo_key, "выбранного раздела")

  await edit_or_reply(
      callback,
      f"Отправьте картинку (фотографию), которая будет отображаться в разделе"
      f" **{cat_name}**:",
      reply_markup=builder.as_markup(),
      state=state,
  )
  await state.set_state(AdminStates.waiting_for_category_photo)
...
async def adm_clear_category_photo(callback: CallbackQuery, state: FSMContext):
  ...
  await state.clear()
  await callback.answer("Фотография категории успешно удалена!", show_alert=True)
  await adm_photos_menu(callback, state)
...
async def adm_save_category_photo(message: Message, state: FSMContext):
  ...
  if not photo_key:
    await message.answer("Ошибка сессии. Повторите попытку через админ-панель.")
    await state.clear()
    return

  photo_id = message.photo[-1].file_id
  update_setting(photo_key, photo_id)
  await state.clear()
  await message.answer("Фотография для выбранного раздела успешно обновлена!")
  await show_admin_menu(message, state)
...
async def adm_edit_balance_get_user(message: Message, state: FSMContext):
  ...
  try:
    cursor.execute(
        "SELECT balance, username FROM users WHERE user_id = ?", (target_id,)
    )
    res = cursor.fetchone()
  finally:
    conn.close()
...
async def adm_completed_orders(callback: CallbackQuery, state: FSMContext):
  ...
  for o_id, u_id, usdt, rub, sent_rub in orders:
    text += (
        f"🆔 **#{o_id}** | ID Клиента: `{u_id}` | `{usdt} USDT` ➔"
        f" `{sent_rub or rub} ₽`\n"
    )
...
async def adm_export_users(callback: CallbackQuery):
  ...
  for uid, uname, bal, deals in users:
    ...
    file_content += (
        f"{str(uid):<15} | {uname_str:<20} | {f'{bal} USDT':<12} |"
        f" {str(deals):<6}\n"
    )
...
async def adm_export_system_logs(callback: CallbackQuery):
  ...
  try:
    cursor.execute(
        "SELECT id, user_id, username, action, created_at FROM system_logs"
        " ORDER BY id DESC"
    )
  ...
  for l_id, uid, uname, action, created in logs:
    uname_str = f"@{uname}" if uname else "none"
    file_content += (
        f"{f'#{l_id}':<6} | {str(uid):<15} | {uname_str:<18} |"
        f" {str(action):<35} | {str(created):<19}\n"
    )
