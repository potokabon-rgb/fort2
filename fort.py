...
async def edit_or_reply(event: Message | CallbackQuery, text: str, reply_markup=None, state: FSMContext = None):
    ...
    if isinstance(event, CallbackQuery):
        try:
            if main_msg_id and main_msg_id != event.message.message_id:
                try:
                    ...
                    pass
            msg = await event.message.edit_text(text, reply_markup=reply_markup, parse_mode="Markdown",
            ...
            try:
                if main_msg_id:
                    try:
                        ...
                        pass
                msg = await event.message.answer(text, reply_markup=reply_markup, parse_mode="Markdown",
                                                 disable_web_page_preview=True)
                if state:
                    await state.update_data(main_msg_id=msg.message_id)
        ...
        try:
            new_msg = await event.answer(text, reply_markup=reply_markup, parse_mode="Markdown",
                                         disable_web_page_preview=True)
            if state:
...
async def send_log(user_id: int, username: str, action: str):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("INSERT INTO system_logs (user_id, username, action) VALUES (?, ?, ?)",
...
def main_menu_kb():
    ...
    builder.button(text="↗️ Актуальные курсы", callback_data="exchange_rate", icon_custom_emoji_id=5994378914636500516)
...
async def user_profile(callback: CallbackQuery, state: FSMContext):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT balance, total_deals, completed_deals, joined_date FROM users WHERE user_id = ?",
                       (user_id,))
        res = cursor.fetchone()
    finally:
        conn.close()
...
async def process_sell_amount(message: Message, state: FSMContext):
    ...
    if user_balance > 0:
        ...
        # Если баланса нет, сразу переходим к запросу чека
...
@router.callback_query(F.data.in_({"use_balance_yes", "use_balance_no"}),
...
async def process_balance_choice(callback: CallbackQuery, state: FSMContext):
    ...
    if callback.data == "use_balance_yes" and user_balance >= amount_usdt:
        # Сразу создаем заявку без чека, так как всё покрыто балансом
        conn = sqlite3.connect("usdt_exchange.db")
        cursor = conn.cursor()
        try:
...
async def take_order_handler(callback: CallbackQuery):
    ...
    try:
        ...
        if status not in ('Ожидает администратора', 'Ожидает реквизиты'):
            await callback.answer(f"⚠️ Заявка уже обработана или находится в статусе: {status}", show_alert=True)
            return

        cursor.execute("UPDATE orders SET status = 'В работе', admin_id = ? WHERE id = ?",
...
async def process_admin_payout_amount(message: Message, state: FSMContext):
    ...
    if not order_id:
        await edit_or_reply(message, "❌ Сессия обработки заказа не найдена.", reply_markup=menu_button_kb(),
                            state=state)
        await state.clear()
        return
    ...
    try:
        ...
        if sent_rub < expected_rub:
            ...
            cursor.execute(
                "UPDATE orders SET sent_rub = ?, remainder_usdt = ?, status = 'Ожидает решения по остатку' WHERE id = ?",
            ...
            user_text = (
                f"⚠️ **Выплата по заявке #{order_id} частичная!**\n\n"
            ...
            try:
                await bot.send_message(chat_id=client_id, text=user_text, reply_markup=builder.as_markup(),
                                       parse_mode="Markdown")
                await edit_or_reply(message,
                                    f"⚠️ Перевод зафиксирован частично (`{sent_rub} ₽`). Запрос решений отправлен клиенту #{client_id}.",
                                    state=state)
            except Exception as e:
                logging.error(f"Ошибка уведомления клиента: {e}")
        else:
            cursor.execute("UPDATE orders SET sent_rub = ?, status = 'Ожидает подтверждения' WHERE id = ?",
                           (sent_rub, order_id))
            conn.commit()

            await finalize_payout(bot, client_id, sent_rub, order_id)
            await edit_or_reply(message, f"✅ Выплата по заявке #{order_id} на сумму {sent_rub} ₽ проведена.",
                                state=state)
    finally:
        conn.close()
...
async def process_user_remainder_choice(callback: CallbackQuery, state: FSMContext):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT remainder_usdt, sent_rub FROM orders WHERE id = ? AND user_id = ?",
                       (order_id, user_id))
        res = cursor.fetchone()
...
async def skip_review_handler(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    await edit_or_reply(callback, "💎 Спасибо за обмен! Будем рады сотрудничать снова.", reply_markup=main_menu_kb(),
                        state=state)
...
async def process_user_review(message: Message, state: FSMContext):
    ...
    await edit_or_reply(message, "✅ **Спасибо за ваш отзыв!** Он передан в публичный канал отзывов.",
                        reply_markup=main_menu_kb(), state=state)
...
async def confirm_no_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await send_log(user_id, callback.from_user.username, f"Сообщил о проблеме с выплатой по заявке #{order_id}")
    ...
    instruction_text = (
        f"⚠️ **Фиксация проблемы по заявке #{order_id}**\n\n"
        f"‼️ **Для решения диспута запишите видео экрана:**\n\n"
...
async def exchange_rate_handler(callback: CallbackQuery, state: FSMContext):
    await send_log(callback.from_user.id, callback.from_user.username, "Посмотрел курс")
    r1 = get_setting("rate_tier_1")
    r2 = get_setting("rate_tier_2")
    r3 = get_setting("rate_tier_3")
    lim1 = get_setting("tier_limit_1")
...
async def history_handler(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        cursor.execute(
            "SELECT COUNT(*), SUM(CASE WHEN status = 'Завершено' THEN 1 ELSE 0 END) FROM orders WHERE user_id = ?",
            (user_id,))
        ...
        cursor.execute(
            "SELECT id, amount_usdt, amount_rub, status FROM orders WHERE user_id = ? ORDER BY id DESC LIMIT 5",
            (user_id,))
...
async def view_order_details(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        cursor.execute(
            "SELECT amount_usdt, amount_rub, status, check_link, phone, bank, fio, created_at FROM orders WHERE id = ? AND user_id = ?",
    ...
    text = (
        ...
        f"💵 Сумма: `{usdt} USDT`\n"
        f"💰 Ожидается к выплате: `{rub} ₽`\n"
        f"📞 Телефон: `{phone}`\n"
        f"🏦 Банк: `{bank}`\n"
        f"👤 ФИО: `{fio}`\n"
        f"📌 Текущий статус: `{status}`\n"
        f"🔗 Чек: {link}\n"
        f"⏱ Время создания: `{created}`"
    )
...
async def adm_rates_menu(callback: CallbackQuery, state: FSMContext):
    ...
    await edit_or_reply(callback, "⚙️ **Управление курсами и диапазонами:**", reply_markup=builder.as_markup(),
                        state=state)
...
async def adm_pending_orders(callback: CallbackQuery, state: FSMContext):
    ...
    for o_id, u_id, usdt, rub, phone, bank, fio, status, check_link in orders:
        req_info = f"📞 `{phone}` | 🏦 `{bank}` | 👤 `{fio}`" if phone else "⚠️ *Реквизиты еще не введены*"
...
async def adm_broadcast_start(callback: CallbackQuery, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    builder.button(text="🔙 Отмена", callback_data="adm_menu", icon_custom_emoji_id=5778527486270770928)
    await edit_or_reply(callback, "📢 Отправьте текст для рассылки всем пользователям:",
...
async def adm_set_limit_1_start(callback: CallbackQuery, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    builder.button(text="🔙 Отмена", callback_data="adm_rates_menu", icon_custom_emoji_id=5778527486270770928)
    await edit_or_reply(callback, "📊 Введите верхнюю границу для Tier 1 (например, `6.0`):",
                        reply_markup=builder.as_markup(), state=state)
...
async def adm_set_limit_2_start(callback: CallbackQuery, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    builder.button(text="🔙 Отмена", callback_data="adm_rates_menu", icon_custom_emoji_id=5778527486270770928)
    await edit_or_reply(callback, "📊 Введите верхнюю границу для Tier 2 (например, `20.0`):",
                        reply_markup=builder.as_markup(), state=state)
